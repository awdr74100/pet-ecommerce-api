...
15. GET 取得訂單列表 => /api/admin/orders
...
17. GET 取得產品列表 => /api/products

    - (200):true - { products }
    - (500):false - error.message

18. GET 取得單一產品細節 => /api/products/:id
...
21. POST 用戶登出 => /api/user/signout

    - (200):true - 已登出

22. POST 檢查用戶是否持續登入 => /api/user/check

    - (200):true - interval < 10 ? 刷新 : 不刷新
    - (200):false - 未帶有訪問令牌
    - (200):false - 無效的訪問令牌

23. POST 密碼重置 => /api/user/reset
...
25. GET 取得購物車產品列表 => /api/user/cart

    - (200):true - { cart }
    - (200):true - { cart } (x 樣商品遭下架或被移除)
    - (500):false - error.message

26. PATCH 修改購物車產品購買數量 => /api/user/cart/:id
...
27. DELETE 刪除購物車產品 (接受批次處理) => /api/user/cart/:id
...
31. PATCH 取消訂單 => /api/user/orders/:id
...
32. GET 取得訂單列表 => /api/user/orders

    - (200):true - { orders }
    - (500):false - error.message

33. GET 取得某一筆訂單 => /api/user/orders/:id
